...
from verl.utils.dataset.universal_multiround import (MultiRoundGenerator,
                                                     QwenMessages2Inputs)
from verl.utils.dataset.universal_multiround_sf import MultiRoundGeneratorSF
...
class RayTrajDAPOSFTrainer(RayPPOTrainer):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        traj_batch: DataProto = DataProto.from_single_dict(batch_dict)
                        mr_gen = MultiRoundGeneratorSF(traj_batch, rollout_n=self.config.actor_rollout_ref.rollout.n, msg_man=self.msg_man, patch_threshold=self.config.algorithm.patch_threshold,actions_only=self.config.algorithm.actions_only)
                        for _step, (step_batch,sf_batch) in enumerate(tqdm(mr_gen.fetch_batch(), desc="Rollout batched steps")):
                        # pop those keys for generation
                            total_step_num += len(step_batch["input_ids"])
                            step_batch = DataProto.from_single_dict(step_batch)
                            sf_batch_origin = DataProto.from_single_dict(sf_batch)
                            batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
                            non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
                            ...
                            sf_batch_padded, pad_size = pad_dataproto_to_divisor(sf_batch, self.actor_rollout_wg.world_size)
                            if not self.async_rollout_mode:
                                gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch_padded) # TODO do not need pad
                                sf_batch_output = self.actor_rollout_wg.generate_sequences(sf_batch_padded)
                            else:
                                self.async_rollout_manager.wake_up()
                                gen_batch_output = self.async_rollout_manager.generate_sequences(gen_batch_padded)
                                sf_batch_output = self.async_rollout_manager.generate_sequences(sf_batch_padded)
                                self.async_rollout_manager.sleep()
                            gen_batch_output = gen_batch_output.slice(0, gen_size)
                            sf_batch_output = sf_batch_output.slice(0, gen_size)
                            timing_raw.update(gen_batch_output.meta_info["timing"])
                            gen_batch_output.meta_info.pop("timing", None)

                           
                            step_batch = step_batch.union(gen_batch_output)
                            sf_batch = sf_batch_origin.union(sf_batch_output)
                            ...
                            # set response
                            error_step_num += mr_gen.apply_response(step_batch,sf_batch)
