...
class NaiveChatCompletionScheduler(ChatCompletionScheduler):
    ...
    async def generate_sequences(self, batch: DataProto, **sampling_params) -> DataProto:
        ...
        async def callback(completions: ChatCompletion, info: Dict[str, Any], exception: Exception):
            assert exception is None, (completions, info, f"exception: {exception}")
    ...
    def _postprocess(self, batch: DataProto, batch_conversations: List[List[List[Dict[str, str]]]], n: int) -> DataProto:
        ...
        prompts = [self.processor.apply_chat_template(prompt, add_generation_prompt=True, tokenize=False) for prompt in batch.non_tensor_batch["raw_prompt"]]
        ...
        sequences = [self.processor.apply_chat_template(conversation, add_generation_prompt=False, tokenize=False) for conversation in batch_conversations]
