...
DAPO_THRESHOLD=0.3
PATCH_THRESHOLD=2
EXPERIMENT_NAME="qwenvl32b_uis1_DAPO_${DAPO}_${DAPO_THRESHOLD}_patch_${PATCH_THRESHOLD}_gamma_${GAMMA}"
if [ $MASTER_ADDR ];then
    echo $MASTER_ADDR
    echo $MASTER_PORT
    echo $WORLD_SIZE
...
if [ "$RANK" == "0" ]; then
    ...
    python3 -m verl.trainer.main_dapo \
        ...
        actor_rollout_ref.model.path=/checkpoints/Qwen/Qwen2.5-VL-32B-Instruct \
        actor_rollout_ref.actor.optim.lr=1e-6 \
        actor_rollout_ref.actor.use_torch_compile=False \
        actor_rollout_ref.model.use_remove_padding=True \
        actor_rollout_ref.actor.ppo_mini_batch_size=${BATCH_SIZE} \
        actor_rollout_ref.actor.use_fixed_num_mini_batches=true \
        actor_rollout_ref.actor.fixed_num_mini_batches=1 \
        ...
        algorithm.filter_groups.std_DAPO_THRESHOLD=$DAPO_THRESHOLD \
