...
def convert_fc_action_to_json_action(
    ...
    if action_type == 'open_app':
        ...
        x, y = int(action_json['coordinate'][0] / 1000 * width), int(action_json['coordinate'][1] / 1000 * height)
        ...
        start_x, start_y = int(action_json['coordinate'][0] / 1000 * width), int(action_json['coordinate'][1] / 1000 * height)
        end_x, end_y = int(action_json['coordinate2'][0] / 1000 * width), int(action_json['coordinate2'][1] / 1000 * height)
...
class MobileAgentV3_M3A(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    self.info_pool.width = 1080
    self.info_pool.height = 2400
    ...
    try:
      converted_action = convert_fc_action_to_json_action(action_object_str, self.info_pool.width, self.info_pool.height)
      self.info_pool.action_pool.append(action_object_str)
