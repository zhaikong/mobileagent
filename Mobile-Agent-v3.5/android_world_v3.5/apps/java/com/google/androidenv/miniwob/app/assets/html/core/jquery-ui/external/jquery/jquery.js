...
var Sizzle =
    ...
    (function(window) {
       ...
       * than Expr.cacheLength) deleting the oldest entry
       */
                  ...
                  if (parent) {
                    ...
                    if (simple) {
                      while (dir) {
                        ...
                        // Reverse direction for :only-* (if we haven't yet done
                        // so)
                        start = dir =
          ...
          var temp, i, elem, preMap = [], postMap = [],
              ...
              multipleContexts(
                          ...
                          context.nodeType ? [context] : context, []),
                             ...
                             // for seed-results synchronization
        ...
        addHandle('type|href|height|width', function(elem, name, isXML) {
          if (!isXML) {
            return elem.getAttribute(
                name, name.toLowerCase() === 'type' ? 1 : 2);
          }
        });
      }
 ...
 * added after the list has been fired right away with the latest "memorized"
      ...
      if (jQuery.type(elem) === 'object') {
        ...
        if (!support.tbody) {
          ...
          elem = tag === 'table' && !rtbody.test(elem) ?
              ...
              wrap[1] === '<table>' && !rtbody.test(elem) ? tmp :
                                                            0;
...
jQuery.event = {
  ...
  add: function(elem, types, handler, data, selector) {
    var tmp, events, t, handleObjIn, special, eventHandle, handleObj, handlers,
        type, namespaces, origType, elemData = jQuery._data(elem);
  ...
  remove: function(elem, types, handler, selector, mappedTypes) {
    var j, handleObj, tmp, origCount, t, events, special, handlers, type,
        namespaces, origType,
  ...
  trigger: function(event, data, elem, onlyHandlers) {
    var handle, ontype, cur, bubbleType, special, tmp, i,
  ...
  simulate: function(type, elem, event) {
    var e = jQuery.extend(new jQuery.Event(), event, {
      ...
      // function since `originalEvent` should point to the original event for
    ...
    function(orig, fix) {
      jQuery.event.special[orig] = {
        ...
        handle: function(event) {
          ...
          // target. NB: No relatedTarget if the mouse left/entered the browser
          // window
          if (!related ||
...
function domManip(collection, args, callback, ignored) {
  ...
  if (l) {
    fragment = buildFragment(
        args, collection[0].ownerDocument, false, collection, ignored);
...
jQuery.extend({
  ...
  cleanData: function(elems, /* internal */ forceAcceptData) {
    var elem, type, id, data,
        i = 0, internalKey = jQuery.expando, cache = jQuery.cache,
...
jQuery.each(
    ...
    function(name, original) {
      jQuery.fn[name] = function(selector) {
        var elems, i = 0, ret = [], insert = jQuery(selector),
...
support.boxSizing = div.style.boxSizing === '' ||
    div.style.MozBoxSizing === '' || div.style.WebkitBoxSizing === '';
  ...
  for (; index < length; index++) {
    ...
    if (show) {
      ...
      if (display && display !== 'none' || !hidden) {
        jQuery._data(
            elem, 'olddisplay', hidden ? display : jQuery.css(elem, 'display'));
      }
    }
  }
...
jQuery.cssHooks.marginRight =
    addGetHookIf(support.reliableMarginRight, function(elem, computed) {
      if (computed) {
        return swap(
            elem, {'display': 'inline-block'}, curCSS, [elem, 'marginRight']);
      }
    });
        ...
        support.reliableMarginLeft, function(elem, computed) {
          if (computed) {
            return (
                       ...
                       // in IE throws an error Support: IE8 only
...
function Animation(elem, properties, options) {
  var result, stopped,
      ...
      animation = deferred.promise({
        ...
        opts: jQuery.extend(
            true, {specialEasing: {}, easing: jQuery.easing._default}, options),
  ...
  stop: function(type, clearQueue, gotoEnd) {
    ...
    return this.each(function() {
      var dequeue = true, index = type != null && type + 'queueHooks',
          timers = jQuery.timers, data = jQuery._data(this);
    ...
    type: {
      set: function(elem, value) {
        if (!support.radioValue && value === 'radio' &&
          ...
          // in IE8-9 Reset value to default in case type is set after value
          // during creation
          var val = elem.value;
...
jQuery.each(jQuery.expr.match.bool.source.match(/\w+/g), function(i, name) {
  ...
  if (getSetInput && getSetAttribute || !ruseDefault.test(name)) {
    ...
    attrHandle[name] = function(elem, name, isXML) {
      if (!isXML) {
        return elem[jQuery.camelCase('default-' + name)] ? name.toLowerCase() :
                                                           null;
      }
    };
  }
...
if (!getSetAttribute) {
  ...
  attrHandle.id = attrHandle.name =
      attrHandle.coords = function(elem, name, isXML) {
        var ret;
        if (!isXML) {
...
if (!support.style) {
  jQuery.attrHooks.style = {
    get: function(elem) {
      ...
      // .toLowerCase() .cssText, that would destroy case sensitivity in URL's,
      // like in "background"
...
jQuery.parseJSON = function(data) {
  ...
  // nothing remains after removing valid tokens
  return str &&
          !jQuery.trim(str.replace(
              rvalidtokens,
      ...
      return {
        send: function(headers, complete) {
          ...
          for (i in headers) {
            ...
            // setting request header to a null-value.
            //
          ...
          callback = function(_, isAbort) {
            ...
            if (responses) {
              complete(
                  status, statusText, responses, xhr.getAllResponseHeaders());
            }
          };
        ...
        .done(function(responseText) {
          ...
          self.html(
              selector ?
                  ...
                  // dummy div Exclude scripts to avoid IE 'Permission Denied'
                  // errors
                  jQuery('<div>')
...
jQuery.fn.extend({
  offset: function(options) {
    ...
    var docElem, win, box = {top: 0, left: 0}, elem = this[0],
