...
cubePuzzle.reset =
    function() {
  document.getElementsByClassName('active')[0].setAttribute(
      'class', 'cube-image');
  ...
  document.getElementsByClassName('cube-image')[5].setAttribute(
      'class', 'cube-image active');
  ...
  var prefix = (function() {
    var styles = window.getComputedStyle(document.documentElement, ''),
        pre =
            (Array.prototype.slice.call(styles).join('').match(
                 /-(moz|webkit|ms)-/) ||
             (styles.OLink === '' && ['', 'o']))[1],
  ...
  Viewport.prototype.animate = function() {
    ...
    if (this.down) {
      this.torqueX = this.torqueX * this.sensivityFade +
          (this.distanceX * this.speed - this.torqueX) * this.sensivity;
      this.torqueY = this.torqueY * this.sensivityFade +
    ...
    if (this.positionY != this.previousPositionY ||
  ...
  Cube.prototype.rotateSides =
      function() {
    var viewport = this.viewport;
    if (viewport.positionY > 90 && viewport.positionY < 270) {
      this.sides[0]
          .getElementsByClassName('cube-image')[0]
          .style[userPrefix.js + 'Transform'] =
    ...
    while (i > 0 && --i) {
      this.sides[i]
          .getElementsByClassName('cube-image')[0]
          .style[userPrefix.js + 'Transform'] = 'rotate(' + deg + ')';
    }
  } Cube.prototype.sideChange =
              function() {
    ...
    this.sides[this.viewport.currentSide - 1]
        .getElementsByClassName('cube-image')[0]
