...
$miniwob.resetRefCode =
  ...
  function getDOMInfoOfElement(element) {
    ...
    if (textOnly) {
      answer.text = filteredChildNodes
                        .map(function(x) {
                          return x.data.trim();
                        })
                        .join(' ');
      ...
      filteredChildNodes.forEach(function(child) {
        if (child instanceof Text) {
          ...
          if (child !== undefined) answer.children.push(child);
        }
      });
    }
    return answer;
  ...
  function addDOMInfosOfTextNode(textNode, collection) {
    ...
    while (start < absolute_end) {
      ...
      var end_lower_bound = start + 1, end_upper_bound = absolute_end,
      ...
      while (end_lower_bound <= end_upper_bound) {
        ...
        if (range.getClientRects().length == 1) {
          end_lower_bound = end + 1;
              ...
              end_lower_bound + 5,
              Math.floor((end_lower_bound + end_upper_bound) / 2));
        } else {
        ...
        if (itr++ > 1000)
          throwTextNodeError('Text node computation stuck in an infinite loop');
      }
      range.setEnd(textNode, end);
      var rects = range.getClientRects();
      if (rects.length !== 1)
        throwTextNodeError('Text node computation incorrect');
  ...
  try {
    ...
    return err.message;
            ...
            // Add event listeners
            $miniwob.recorder = {} $miniwob.recorder.LISTENERS = [
  'click',
  'dblclick',
  'mousedown',
  'mouseup',
...
$miniwob.recorder.setup =
  ...
  $miniwob.recorder.addState(event, {
    ...
    'charCode': event.charCode,
                                    ...
                                    $miniwob.recorder.onscroll = function(
                                        event) {
  // Scroll is super redundant; only keep the first one
  if ($miniwob.recorder.data.states.length) {
    var lastState =
        $miniwob.recorder.data.states[$miniwob.recorder.data.states.length - 1];
    if (lastState.action && lastState.action.type === 'scroll') return;
    //$miniwob.recorder.data.states.pop();     // <-- use this for keeping the
    //last one
  }
  $miniwob.recorder.addState(event, {
    'type': 'scroll',
